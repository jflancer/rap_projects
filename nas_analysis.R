...
search_song("Cherry Wine")
get_song_meta(3780016)$album_id
# Nas Albums
# Illmatic 7013
# It Was Writtern 7040
# I Am... 7011
# Nastradamus 7015
# Stillmatic 7029
# The Lost Tapes 7014
# God's Son 7006
# Street's Disciple 7001
# Hip Hop is Dead 7009
# Nas 159773
# Distant Relatives 333386
# Life is Good 12283
# NASIR 429552

kanye_albums <- c(7013,7040,7011,7015,7029,7014,7006,7001,7009,159773,333386,12283,429552)
...
feelings_prop$album_name_fact <- ordered(feelings_prop$album_name, 
                                        levels = c("Illmatic",
                                                   ...
                                                   "Life Is Good",
                                                   "NASIR"))
feelings_prop$album_name_fact <- revalue(feelings_prop$album_name_fact, c("The Nigger Tape" = "Untitled"))

ggplot(feelings_prop, aes(sentiment, proportion, fill = sentiment)) +
  geom_col(position = "stack") +
  facet_wrap(~album_name_fact, nrow = 3) +
  theme_bw() +
  theme(axis.title.x = element_blank(),
        axis.text.x = element_blank()) +
  coord_polar() +
  ggtitle("Nas", subtitle = "Sentiment Analysis of Nas's Albums using NRC Lexicon") +
...
temp <- data.frame(album_numb = 1:13, album_name = levels(feelings_prop$album_name_fact))
...
moving_data$album_name <- ordered(moving_data$album_name, 
                                  levels = c("Illmatic",
                                             ...
                                             "Life Is Good",
                                             "NASIR"))
moving_data$album_name <- revalue(moving_data$album_name, c("The Nigger Tape" = "Untitled"))
...
ggplot(moving_data2, aes(ordering, moving_avg)) +
  ...
  facet_wrap(~album_name, nrow =3, scales = "free_x") +
  theme_classic() +
  theme(axis.title.x = element_blank(),
        axis.text.x = element_blank()) +
  ...
  ylim(-5,5) +
  ggtitle("Nas",
          subtitle = "Moving Average Sentiment of Nas's Albums using AFINN Lexicon\nSong Starts in Blue")
